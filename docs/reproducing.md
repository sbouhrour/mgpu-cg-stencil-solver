...
|---|---|---|---|
| SpMV vs cuSPARSE CSR (A100, 10k×10k): **2.05×** (CUDA 12.8), **1.84×** (CUDA 13.0) | [Results](results.md#2d-spmv-format-comparison) | `./bin/generate_matrix 10000 matrix/stencil_10000x10000.mtx`<br>`./bin/spmv_bench matrix/stencil_10000x10000.mtx --mode=cusparse-csr`<br>`./bin/spmv_bench matrix/stencil_10000x10000.mtx --mode=stencil5-csr` | `Execution time` of `stencil5-csr` (3.31 ms) vs `cusparse-csr` (6.80 ms with CUDA 12.8, 6.10 ms with CUDA 13.0). The ratio depends on the cuSPARSE version the binary links against (`ldd bin/spmv_bench \| grep cusparse`). |
